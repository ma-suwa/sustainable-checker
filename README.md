...
- **各領域で意図的に1〜2件のまま**とした項目：外部計測（Core Web Vitals・表示速度）、実機操作（キーボード）、URL確認のみ（サイトマップ・アクセシビリティ方針・各種ポリシー）、複数ページ横断（階層の一貫性）、ブランド映像ヒーロー（事業3秒）など、スクリーンショット1枚では実証できないものは無理に埋めていない。
